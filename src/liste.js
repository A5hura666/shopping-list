...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function afficherListeCourse(panier) {
    ...
    panier.forEach((produit, index) => {
      ...
      const sousTotal = (
        produit.prix_unitaire * produit.quantite_stock
      ).toFixed(2);
    ...
    totalGeneral.textContent =
      total > 0 ? `Total général: ${total.toFixed(2)} €` : "";

    document.getElementById("vider-liste").style.display =
      panier.length > 0 ? "block" : "none";
  ...
  function supprimerProduit(event) {
    ...
    ajusterStockProduit(
      produitSupprime.nom,
      produitSupprime.quantite_stock,
      true
    ); // Récupérer le stock
  ...
  function mettreAJourQuantite(event) {
    ...
    if (!isNaN(nouvelleQuantite) && nouvelleQuantite > 0) {
      ...
      if (
        nouvelleQuantite >
        produitStock.quantite_stock + produit.quantite_stock
      ) {
      ...
      ajusterStockProduit(
        produit.nom,
        nouvelleQuantite - produit.quantite_stock
      );
